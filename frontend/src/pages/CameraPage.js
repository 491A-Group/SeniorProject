...
export default function CameraPage({changePage, setSource, source, setPredict, prediction}) {
  ...
  const changeSource = () => {
    setSource(previousValue => {
      ...
      .then(data => {
        setPredict(JSON.stringify(data[0]));
        console.log("DATA: " + data[0]);
        changePage(JSON.stringify(data[0]))
      })
      .catch((error) => {
